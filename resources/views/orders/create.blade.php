...
<div class="container">
    ...
    <form action="{{ route('orders.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="loading_place" name="loading_place" required>
        </div>

        <div class="mb-3">
            <label for="unloading_place" class="form-label">Місце розвантаження</label>
            <input type="text" class="form-control" id="unloading_place" name="unloading_place" required>
        </div>

        <div class="mb-3">
            <label for="loading_date" class="form-label">Дата завантаження</label>
            <input type="date" class="form-control" id="loading_date" name="loading_date" required>
        </div>

        <div class="mb-3">
            <label for="unloading_date" class="form-label">Дата розвантаження</label>
            <input type="date" class="form-control" id="unloading_date" name="unloading_date" required>
        </div>

        <div class="mb-3">
            <label for="cargo_type" class="form-label">Тип вантажу</label>
            <input type="text" class="form-control" id="cargo_type" name="cargo_type" required>
        </div>

        <div class="mb-3">
           <label for="grain_quantity" class="form-label">Кількість (тонни)</label>
           <input type="number" class="form-control" id="grain_quantity" name="grain_quantity" step="0.1" required value="{{ old('grain_quantity', $order->grain_quantity ?? '') }}">
        </div>

        <div class="mb-3">
           <label for="price_per_ton" class="form-label">Ціна за тонну</label>
           <input type="number" class="form-control" id="price_per_ton" name="price_per_ton" step="0.1" required value="{{ old('price_per_ton', $order->price_per_ton ?? '') }}">
        </div>
...
<script>
    ...
    function calculateTransportCost() {
        const quantity = parseFloat(quantityInput.value) || 0;  // Виправлено тут
